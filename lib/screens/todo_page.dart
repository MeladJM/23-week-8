    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        return ListView.builder(
          ...
          itemBuilder: ((context, index) {
            Todo todo = Todo.fromJson(    //gagaa ng todo object from the document
              snapshot.data?.docs[index].data() as Map<String,dynamic>);
            todo.id = snapshot.data?.docs[index].id;
        return Dismissible(
        key: Key(todo.id.toString()),

        //if may error ididismiss or idedelete
        onDismissed: (direction) {
        //context.read<TodoListProvider>().deleteTodo(todo.title);
        // ScaffoldMessenger.of(context).showSnackBar(
        // SnackBar(content: Text('${todo.title}dismissed')));
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          showDialog(
            ...
            builder: (BuildContext context) => TodoModal(
              type: 'Add',
              item: null, // Flag to identify that this particular modal is for add
            ),
          );
        },
